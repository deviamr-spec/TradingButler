...
class BotController(QObject):
    ...
    def handle_tick_data(self, tick_data):
        """Handle tick data from analysis worker"""
        try:
            self.last_tick = tick_data
            self.signal_market_data.emit(tick_data)
        except Exception as e:
            self.log_message(f"Tick data handling error: {e}", "ERROR")
    
    def handle_indicators_update(self, indicators):
        """Handle indicators update from analysis worker"""
        try:
            self.last_indicators = indicators
            self.signal_indicators_update.emit(indicators)
        except Exception as e:
            self.log_message(f"Indicators update error: {e}", "ERROR")
